...
static int do_procs(int fd) {
	...
	// get max allowed size for message
	elemsz = ioctl(fd, SCULL_IOCGETELEMSZ);
       	buf = (char*)malloc(elemsz + 1); // allocate elemsz bytes + 1 for null term
